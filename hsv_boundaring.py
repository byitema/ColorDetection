...
if __name__ == "__main__":
    ...
    #считываем эталонные изображения
    print("Построение библиотеки моделей-эталонов...")

    greenSquare = ImageModel(cv2.imread('greenSquare.png'))
    print("\nМодель эталона №1 (зелёный квадрат):")
    greenSquare.model()

    redCircle = ImageModel(cv2.imread('redCircle.png'))
    print("\nМодель эталона №2 (красный круг):")
    redCircle.model()

    yellowTriangle = ImageModel(cv2.imread('yellowTriangle.png'))
    print("\nМодель эталона №3 (жёлтый треугольник):")
    yellowTriangle.model()

    etalon = {
        "зелёный квадрат": greenSquare,
        "красный круг": redCircle,
        "жёлтый треугольник": yellowTriangle,
    }

#считываем изображение
    print("--------------------------------------------------------")
    print("Построение модели входного изображения...")

    img = ImageModel(cv2.imread('redCircle.png'))
    print("\nМодель входного изображения:")
    img.model()

    print("\nПостроение модели входного изображения завершено.")

    #сравниваем с эталонами
    print("--------------------------------------------------------")
    print("Поиск в библиотеке эталонов аналога модели входного изображения...")
    rec = ""
    for key in etalon.keys():
        if img == etalon.get(key):
            rec = key

    if rec != "":
        print("Соответствие найдено: " + rec + ".")
    else:
